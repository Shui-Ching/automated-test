...
### Phase 2 — 後台頁面管理 CRUD（狀態：進行中，2.1～2.3、2.7～2.8（僅頁面列表部分）已完成）

| # | 任務 | 對應 AC |
| :-- | :--- | :--- |
| 2.1 | ~~資料層 `data-store.js`~~ **已完成（頁面列表所需部分）**：只實作列表查詢（模糊比對＋日期區間）與硬刪除，因為目前唯一消費者是頁面列表。唯一值檢核、寫入、IndexedDB 圖片存取待 2.4～2.6 實作新增／編輯頁面時再補 | 全功能共用 |
...
| 2.7 | ~~產出後台三頁的測試契約~~ **僅頁面列表部分已完成**：`testid-map.json` 擴充 page-list、新增 `tests/contract/seed.md`（資料填種方式）與 `tests/contract/assertion-types.md`（回補 Phase 1 缺的檔案）。新增／編輯頁面的契約待 2.4～2.6 完成後再補，PM 端據此撰寫的測試也還剩約 35 條 | — |
| 2.8 | ~~契約檢查測試擴充到後台三頁~~ **僅頁面列表部分已完成**：`contract-check.spec.ts` 已涵蓋 page-list（透過 `seedAdmin` 建立登入狀態才能過 auth-guard），新增／編輯頁面待實作後補 | — |
...
| 2.9 | **派工分類驗證**：分別植入三個 bug（版型排列錯 → 需設計審查／錯誤文案錯字 → 前端／唯一值檢核失效 → 後端），確認派工單各自掛對負責單位 | 後台 CRUD 是第一個同時涵蓋三類 AC 的範圍，這裡才真正回答「能不能自動派對單位」。**限制**：規格的 UI 截圖全部待補，沒有設計基準可比對，所以第三類只能標記為「需人工設計審查」，不能斷定是設計出錯——見 `pm-feedback.md` D-3。**待 2.4～2.6（新增／編輯頁面）完成後才能做**，因為三類 bug 需要涵蓋唯一值檢核與版型排列，這兩者都在新增／編輯頁面 |
