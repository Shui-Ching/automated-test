...
### Phase 2 — 後台頁面管理 CRUD（狀態：進行中，2.1～2.5、2.7～2.8（頁面列表＋新增頁面部分）已完成）

| # | 任務 | 對應 AC |
| :-- | :--- | :--- |
| 2.1 | ~~資料層 `data-store.js`~~ **已完成**：列表查詢、硬刪除、唯一值檢核（`pageNameExists`）、新增寫入（`createPage`）、IndexedDB 圖片 Blob 存取（`saveImageBlob`／`getImageBlob`／`deleteImageBlob`）全部到位。編輯頁所需的 `updatePage` 待 2.6 再補 | 全功能共用 |
...
| 2.5 | ~~欄位驗證共用模組（11 條規則、送出時／離開欄位時兩種時機）~~ **已完成**：`app/assets/js/field-validation.js`，純函式、不碰 DOM，供 2.4 使用、2.6 沿用 | PAG-002/003 AC-B1 展開 |
| 2.6 | 編輯頁面：帶入既有值、還原區塊、唯一值檢核排除本筆、返回不寫入 | PAG-003 AC-P1～P6 |
| 2.7 | ~~產出後台三頁的測試契約~~ **頁面列表＋新增頁面已完成**：`testid-map.json` 擴充 page-create（`page-form-*` 前綴，供 2.6 編輯頁沿用同一批 testid）。編輯頁面的契約待 2.6 完成後再補 | — |
| 2.8 | ~~契約檢查測試擴充到後台三頁~~ **頁面列表＋新增頁面已完成**：`contract-check.spec.ts` 已自動涵蓋 page-create（迭代 `testid-map.json` 的 `pages` 陣列，新頁面免改測試碼），編輯頁面待實作後自動補上 | — |
...
| 2.8c | ~~PAG-ADM-FN-002 共 14／17 條 AC 測試~~ **已完成**：`tests/specs/pag-adm-fn-002.spec.ts`，並已回填 `ac-coverage.json` 對應列。以刻意植入 bug（區塊移除邏輯改成永遠刪第一組）驗證 AC-P3 會轉紅，還原後 33 條測試（含 Phase 1、2.8b）全綠。**AC-B1「區塊版型每一組皆不可為空」（id 20）與 AC-P4「再次進入編輯頁時應完整還原」的後半段未覆蓋，原因見下方第 9 節「下一步」** | PAG-002 14／17 條 |
| 2.9 | **派工分類驗證**：分別植入三個 bug（版型排列錯 → 需設計審查／錯誤文案錯字 → 前端／唯一值檢核失效 → 後端），確認派工單各自掛對負責單位 | 後台 CRUD 是第一個同時涵蓋三類 AC 的範圍，這裡才真正回答「能不能自動派對單位」。**限制**：規格的 UI 截圖全部待補，沒有設計基準可比對，所以第三類只能標記為「需人工設計審查」，不能斷定是設計出錯——見 `pm-feedback.md` D-3。**待 2.6（編輯頁面）完成後才能做**，因為三類 bug 需要涵蓋唯一值檢核與版型排列，這兩者都在新增／編輯頁面 |
...
| AC 描述特徵 | 判給 |
| :--- | :--- |
| 版型、排列方向、置左置右、並列、顯示／隱藏樣式 | 需設計審查 + 前端（PM 補上 UI 截圖後才能改判為「設計」） |
...
1. **AC-B1「區塊版型每一組皆不可為空」（`ac-coverage.json` id 20）在目前 UI 上無法用真實使用者操作觸發**：區塊版型單選有規格要求的預設值「左圖右文」，一般人不可能把它清成空值，這條規則因此變成寫了也測不到的防禦性檢核。三個選項：(a) 維持現況，接受這條規則只在程式碼層防禦、不寫自動化測試；(b) 拿掉單選的預設值，讓使用者必須主動選才算數，但這樣會跟規格〈介面欄位定義〉表明寫的「預設值：左圖右文」衝突，要先確認是否要改規格；(c) 用 `page.evaluate` 直接竄改內部狀態硬測，但這脫離真實使用者行為，測試意義存疑。目前採 (a)，未寫測試。
2. **AC-P4「再次進入編輯頁時，編輯器內之粗體與項目清單格式應完整還原」只驗證了前半段**（新增頁儲存後 HTML 正確寫入 storage），後半段需要 2.6 的編輯頁面把內容載回編輯器才能驗證，屆時要回來把 `tests/specs/pag-adm-fn-002.spec.ts` 的這條測試補完整。
