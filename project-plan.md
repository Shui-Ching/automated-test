...
### Phase 0 — 基礎建設與決策（狀態：進行中，0.1～0.3 已完成）

| # | 任務 | 產出／驗收 |
| :-- | :--- | :--- |
| 0.1 | ~~`git init` 並接上 remote~~ **已完成**：已確認本層與下一層無既有 `.git`，`core.quotepath false` 已設，`.gitignore` 已建。尚未 commit／push | — |
...
| 0.4 | 決策：NFR-002 的 30 分鐘逾時、NFR-004 的寫入失敗如何測（見風險 R-1、R-9） | 決議寫回本檔；兩者都需要在 `data-store.js` 留可注入的測試 hook |
...
### Phase 1 — 垂直切片：用「後台登入」驗證整條流程（狀態：未開始）
...
| 1.7 | 寫 `dispatch-reporter.ts`：解析 JSON reporter 輸出 → 產出派工單 Markdown | 失敗時產出含「AC 編號 / 情境 / 預期 / 實得 / 負責單位 / 截圖」的條列 |
...
### Phase 2 — 後台頁面管理 CRUD（狀態：未開始）

| # | 任務 | 對應 AC |
| :-- | :--- | :--- |
| 2.1 | 資料層 `data-store.js`：頁面 CRUD、唯一值檢核、硬刪除、IndexedDB 圖片存取 | 全功能共用 |
| 2.2 | 頁面列表：預設排序、名稱模糊搜尋、日期區間篩選、分頁（10/20/50）、登出 | PAG-001 AC-P1、P2、P5、B1 |
| 2.3 | 刪除確認彈窗 + 硬刪除 + Toast | PAG-001 AC-P3、P4 |
| 2.4 | 新增頁面：欄位、圖文區塊可重複群組（1～3 組）、圖片上傳與預覽、富文本編輯器 | PAG-002 AC-P1～P4、B2、B3 |
| 2.5 | 欄位驗證共用模組（11 條規則、送出時／離開欄位時兩種時機） | PAG-002/003 AC-B1 展開 |
| 2.6 | 編輯頁面：帶入既有值、還原區塊、唯一值檢核排除本筆、返回不寫入 | PAG-003 AC-P1～P6 |
| 2.7 | 產出後台三頁的測試契約，PM 端據此撰寫約 41 條測試 | — |
| 2.8 | 契約檢查測試擴充到後台三頁 | — |
| 2.9 | **派工分類驗證**：分別植入三個 bug（版型排列錯 → 需設計審查／錯誤文案錯字 → 前端／唯一值檢核失效 → 後端），確認派工單各自掛對負責單位 | 後台 CRUD 是第一個同時涵蓋三類 AC 的範圍，這裡才真正回答「能不能自動派對單位」。**限制**：規格的 UI 截圖全部待補，沒有設計基準可比對，所以第三類只能標記為「需人工設計審查」，不能斷定是設計出錯——見 `pm-feedback.md` D-3 |
